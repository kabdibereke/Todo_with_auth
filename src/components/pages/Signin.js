  ...
  return (
    <div className="flex justify-center items-center w-full h-screen bg-[#0D2960]">
        <div className="flex flex-col gap-3 justify-center items-center w-full sm:w-[450px] py-10 bg-white">
           <p className="text-2xl   text-teal-900">Sign In</p>
           <p>{error }</p>
            <form  onSubmit={hundleSubmit} >
                ...
                {loading? <p>{spinner}</p>:<button  className="mt-5 w-20 h-8 bg-teal-900 text-white rounded-md" >Sign In</button>} 
                <p className=" mt-5 text-[16px]  text-teal-900" >Don't have account? <Link to="/signup">Sign Up</Link></p>
            </form>
        </div>
    </div>
  )
